...
//                                    String verification_token = Security.encrypt(loanid);
     ...
     * ----------------------------- */
        ...
        int installmentPeriod
                = Integer.parseInt(repaymentPeriods.get(0).getPeriod());

        /* -----------------------------
     * 3. EMI CALCULATION
     * ----------------------------- */
        double monthlyInstallment
                = calculateMonthlyInstallment(expectedLoanAmount, interestRate, installmentPeriod);
        
        actualMonthlyInstallement = String.valueOf(monthlyInstallment);
        
        if (monthlyInstallment <= 0) {
            msg = new FacesMessage(FacesMessage.SEVERITY_WARN,
                    "Invalid Monthly Installment!", "");
            FacesContext.getCurrentInstance().addMessage(null, msg);
            return;
        }
     ...
     * ----------------------------- */
        ...
        baseCal.add(Calendar.DAY_OF_MONTH, 7); // first installment in 7 days
        Date firstDueDate = baseCal.getTime();
     ...
     * ----------------------------- */
        ...
        while (openingBalance > 0 && monthIndex < installmentPeriod) {
            
            Calendar installmentCal = Calendar.getInstance();
            installmentCal.setTime(firstDueDate);
            installmentCal.add(Calendar.MONTH, monthIndex);
            
            Date installmentDate = installmentCal.getTime();
            String dueDate = ComLib.getDate(installmentDate);
            
            double interest = openingBalance * monthlyRate;
            totalInterestPaid += interest;
            
            double installmentPayment = monthlyInstallment;

            // Adjust last installment
            if (openingBalance + interest < monthlyInstallment) {
                installmentPayment = openingBalance + interest;
            }
            
            double principalAmount = installmentPayment - interest;
            paidCapital += principalAmount;
            totalPayment += installmentPayment;
            
            if (monthIndex == 0) {
                System.out.println("First installment due on: " + firstDueDate);
                
                loanStrucure.add(new LoanCalculatorRecords(
                        monthIndex + 1,
                        ComLib.getDate(firstDueDate),
                        openingBalance,
                        principalAmount,
                        interest,
                        totalInterestPaid,
